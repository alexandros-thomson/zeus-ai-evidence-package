// ================================================================
// Check Status + Parse — v3.0-DEDUP-L1L3
// Last updated: 2026-03-09
// DEDUP guard: Layer 1 ($getWorkflowStaticData) + Layer 3 (Google Sheets)
// Layer 2 (fs) REMOVED — incompatible with n8n 2.x task runners
// Total tracked cases: 19 | Total protocols: 290+
// ================================================================
//
// CHANGES FROM v2.9-DEDUP-L1L3:
//   - Case 18: EFKA-KAKLAMANOU-NAVY-PENSION (Κακλαμάνου confirmed Navy pensioner,
//              Δ' Δ/νση Απονομής Συντάξεων, Row 276)
//   - Case 19: EAD-15006 (Supplement #4, triple-lock with 14288/14441)
//   - Engine version bumped to v3.0
//
// DOWNSTREAM: No changes — IF Node still uses $json.should_send === true
// ================================================================
...
// ── LAYER 1: n8n Static Data (in-memory, survives re-executions) ──
...
// ================================================================
// CASES ARRAY — 19 cases (v2.9 Cases 1-17 + Case 18-19)
// ================================================================
const cases = [
  ...
  // Κακλαμάνου confirmed: "ο θανών ήταν συνταξιούχος του Πολεμικού Ναυτικού"
  // Row 276 — two independent pension paths:
  //   Path A: EFKA Δ' Δ/νση Απονομής Συντάξεων (tm.sint.td.apireas@efka.gov.gr)
  //   Path B: NAT Τμ. Μητρώου Συνταξιούχων 2 (Case 16)
  {
  ...
  // Supplement #4 protocolled 09/03/2026
  // Triple-lock: 14288 + 14441 + 15006
  {
...
// Summary item (filtered out by IF Node since should_send is absent)
...
output.push({
  json: {
    ...
    engine_version: 'v3.0-DEDUP-L1L3',
    guard_layers: 'L1:static_data | L3:google_sheets (L2:fs removed for n8n 2.x compat)',
